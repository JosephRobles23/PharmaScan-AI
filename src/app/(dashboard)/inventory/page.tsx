        ...
        <div className="space-y-4">
            ...
            <div>
                ...
                <p className="text-sm text-[#64748B] mt-0.5">
                    {products.length} productos · {units.length} unidades
                </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                {/* View Toggle */}
                <div className="flex rounded-lg border border-[#E2E8F0] dark:border-[#334155] overflow-hidden h-10">
                    <button
                        onClick={() => setViewMode('summary')}
                        className={`px-3 py-1 text-sm font-medium transition-colors ${viewMode === 'summary'
                    ...
                    <button
                        ...
                        className={`px-3 py-1 text-sm font-medium transition-colors ${viewMode === 'units'
                            ? 'bg-[#0066CC] text-white'
                            : 'bg-white dark:bg-[#1E293B] text-[#64748B] hover:bg-[#F8FAFC] dark:hover:bg-[#0F172A]'
                            }`}
                    >
                        ...
                        <Card>
                            <CardContent className="py-8 text-center">
                                ...
                                <p className="text-sm text-[#64748B]">
                                    {searchQuery ? 'Sin resultados' : 'Escanea productos'}
                                </p>
                            </CardContent>
                        </Card>
                    ) : (
                            ...
                            <Card key={product.id}>
                                <CardContent className="p-0">
                                    <button
                                        ...
                                        className="w-full p-3 flex items-center justify-between text-left hover:bg-[#F8FAFC] dark:hover:bg-[#0F172A] transition-colors"
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-sm text-[#0F172A] dark:text-[#F1F5F9]">
                                                    {product.product_name}
                                                </p>
                                                <p className="text-xs text-[#64748B]">
                                                    {format(new Date(product.last_updated), "dd MMM", { locale: es })}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                    ...
                                    {/* Expanded units */}
                                    {expandedProduct === product.product_name && (
                                        <div className="border-t border-[#E2E8F0] dark:border-[#334155] p-3 bg-[#F8FAFC] dark:bg-[#0F172A]">
                                            {productUnits[product.product_name] ? (
                                                <div className="space-y-2">
                                                    {productUnits[product.product_name].map((unit) => (
                                                        <div
                                                            key={unit.id}
                                                            className="flex items-center justify-between p-2.5 bg-white dark:bg-[#1E293B] rounded-lg"
                                                        >
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="p-1.5 text-[#EF4444] hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors"
                                                                >
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-0.5 text-xs text-[#64748B]">
                                                <span>{unit.product_code || 'N/A'}</span>
                                                <span>·</span>
                                                <span className="flex items-center gap-1">
                                                    <Calendar className="w-3 h-3" />
                                                    {format(new Date(unit.expiration_date), "dd MMM yy", { locale: es })}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
